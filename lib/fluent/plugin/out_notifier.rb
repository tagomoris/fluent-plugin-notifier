...
# </match>
  ...
  class Definition
    ...
    def initialize(element, defaults)
      ...
      @intervals = [
                    (element['interval_1st'].to_f || defaults[:interval_1st]),
                    (element['interval_2nd'].to_f || defaults[:interval_2nd]),
                    (element['interval_3rd'].to_f || defaults[:interval_3rd])
                   ]
      @repetitions = [
                      (element['repetitions_1st'].to_f || defaults[:repetitions_1st]),
                      (element['repetitions_2nd'].to_f || defaults[:repetitions_2nd])
                     ]
    end
